document.addEventListener('DOMContentLoaded', () => {
    ...
    heroVideos.forEach((heroVideo) => {
        ...
        const tryPlay = () => {
            if (heroVideo.ended || heroVideo.dataset.done === '1') return;
            heroVideo.muted = true;
    ...
    const translations = {
        uk: {
            ...
            equipSolIndKicker: 'Промислові рішення', equipSolIndTitle: 'Для бізнесу', equipSolIndMeta: 'Підприємства · офіси · комерція',
            equipSolHomeKicker: 'Побутові рішення', equipSolHomeTitle: 'Для дому', equipSolHomeMeta: 'Будинок · квартира · таунхаус',
        ...
        en: {
            ...
            uc4h: 'Backup Power', uc4p: 'Business protection from outages — instant switching in <20 ms',
            ...
            calcOpt1: 'DAM Arbitrage', calcOpt2: 'Trading', calcOpt3: 'Own Consumption', calcOpt4: 'Backup Power', calcOpt5: 'With Generation',
            ...
            equipTitle: 'Equipment', equipSubtitle: 'Choose a use case, inverter power and battery capacity',
            equipInvLabel: 'Inverter', equipBatLabel: 'Battery',
            equipSolIndKicker: 'Industrial solutions', equipSolIndTitle: 'For business', equipSolIndMeta: 'Factories · offices · commercial',
            equipSolHomeKicker: 'Residential solutions', equipSolHomeTitle: 'For home', equipSolHomeMeta: 'House · apartment · townhouse',
            equipSwipe: 'Swipe between solutions',
    ...
    if (inverterOptions && batteryOptions) {
        const solutions = {
            ...
            home: {
                ...
                defaultBat: 10,
                inverterData: {
                    5:  { efficiency: '≥ 97.6%', pricePerKwh: 520 },
                    8:  { efficiency: '≥ 97.8%', pricePerKwh: 480 },
                    10: { efficiency: '≥ 98.0%', pricePerKwh: 450 },
                    12: { efficiency: '≥ 98.2%', pricePerKwh: 430 }
                },
                protection: 'IP65',
                cycles: '10000',
                batteryType: 'CATL LFP (LiFePO₄)',
                modelPrefix: 'KSTAR BluE',
                badgeKey: 'equipBadgeHome',
                imageAlt: {
                    uk: 'KSTAR BluE — побутова система накопичення енергії',
                    en: 'KSTAR BluE — residential energy storage system'
                }
            }
        };

        const solOrder = ['industrial', 'home'];
        let selectedSol = 'industrial';
        let selectedInv = solutions.industrial.defaultInv;
        let selectedBat = solutions.industrial.defaultBat;

        function currentSolution() {
            return solutions[selectedSol];
        }

        function renderOptionButtons() {
            const sol = currentSolution();
            inverterOptions.innerHTML = sol.inverters.map((value) => {
                const active = value === selectedInv ? ' equip-config__btn--active' : '';
                return `<button type="button" class="equip-config__btn equip-config__btn--inv${active}" data-inv="${value}">${value} kW</button>`;
            }).join('');
            batteryOptions.innerHTML = sol.batteries.map((value) => {
                const active = value === selectedBat ? ' equip-config__btn--active' : '';
        ...
        function updateEquipSpecs() {
            const sol = currentSolution();
            const inv = sol.inverterData[selectedInv];
            if (!inv) return;
            ...
            if (modelEl) modelEl.textContent = `${sol.modelPrefix} — ${selectedInv}kW / ${selectedBat}kWh`;
            if (badgeEl) {
                badgeEl.textContent = t[sol.badgeKey] || badgeEl.textContent;
                badgeEl.dataset.i18n = sol.badgeKey;
            }
            if (photoEl && sol.imageAlt[currentLang]) photoEl.alt = sol.imageAlt[currentLang];
            ...
            const updates = [
                ...
                [protEl, sol.protection],
                [cyclesEl, sol.cycles],
                [batTypeEl, sol.batteryType]
            ];
        ...
        function setSolution(solId) {
            if (!solutions[solId] || solId === selectedSol) return;
            selectedSol = solId;
            const sol = currentSolution();
            selectedInv = sol.defaultInv;
            selectedBat = sol.defaultBat;
        ...
        if (equipSolutions) {
            ...
            let touchX = null;
            equipSolutions.addEventListener('touchstart', (e) => {
                touchX = e.changedTouches[0].clientX;
            }, { passive: true });
            equipSolutions.addEventListener('touchend', (e) => {
        ...
        inverterOptions.addEventListener('click', (e) => {
            ...
            selectedInv = parseInt(btn.dataset.inv, 10);
            updateEquipSpecs();
            flashZone(zoneInv);
        });
        ...
        batteryOptions.addEventListener('click', (e) => {
            ...
            btn.classList.add('equip-config__btn--active');
            selectedBat = parseInt(btn.dataset.bat, 10);
            updateEquipSpecs();
            flashZone(zoneBat);
        });
